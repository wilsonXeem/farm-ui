  ...
  return (
    <Shell>
      ...
      <div className="card mb-4">
        ...
        <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${myPens.length}, 1fr)` }}>
          {myPens.map(pen => {
...
function AccountantDashboard() {
  ...
  const revExpData = last7.map(d => ({
    date: d.slice(5),
    revenue: sales.filter(r => r.date === d).reduce((s, r) => s + r.total, 0),
    expenses: expenses.filter(r => r.date === d).reduce((s, r) => s + r.amount, 0),
  }))

  const expPieData = [
    { name: 'Feed', value: feed.reduce((s, r) => s + r.totalCost, 0) },
    { name: 'Salaries', value: workers.reduce((s, w) => s + w.salary, 0) },
    ...['Fuel', 'Medication', 'Electricity', 'Transport', 'Other'].map(cat => ({
      name: cat,
      value: expenses.filter(e => e.category === cat).reduce((s, r) => s + r.amount, 0),
    })),
  ].filter(d => d.value > 0)
...
function AdminDashboard() {
  const { production, expenses, feed, sales, workers, inventory, pens, mortality } = useFarmStore()
  const t = useTotals()
  const last7 = getLast7Days()
  const today = last7[last7.length - 1]
  ...
  return (
    <Shell>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        ...
        <KpiCard label="Total expenses" value={fmt(t.totalExpenses)} color="red" />
        <KpiCard label="Net profit / loss" value={(t.profit < 0 ? '-' : '') + fmt(Math.abs(t.profit))} color={t.profit >= 0 ? 'green' : 'red'} />
        <KpiCard label="Suggested price" value={fmt(suggestedPrice) + '/egg'} sub="25% margin" color="blue" />
      </div>
            ...
            {pens.map(pen => {
              ...
              return (
                <div key={pen.id} className="bg-stone-50 rounded-lg p-3 text-center">
                  ...
                  <div className="text-base font-medium text-brand-600">{fmtN(pen.totalBirds - deaths)}</div>
                  <div className="text-[10px] text-stone-400">birds</div>
...
function SalesDashboard() {
  ...
  return (
    <Shell>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <KpiCard label="Total revenue" value={fmt(totalRevenue)} color="green" />
        <KpiCard label="Total expenses" value={fmt(totalExpenses)} color="red" />
        <KpiCard label="Unpaid debts" value={fmt(unpaidDebt)} color="red" />
        <KpiCard label="Transactions" value={String(sales.length)} />
        <KpiCard label="Revenue today" value={fmt(todaySales)} color="green" />
        <KpiCard label="Expenses today" value={fmt(todayExp)} color="red" />
        <KpiCard label="Net (revenue - expenses)" value={(totalRevenue - totalExpenses) >= 0 ? fmt(totalRevenue - totalExpenses) : '-' + fmt(Math.abs(totalRevenue - totalExpenses))} color={(totalRevenue - totalExpenses) >= 0 ? 'green' : 'red'} />
        <KpiCard label="Paid sales" value={String(sales.filter(s => s.status === 'Paid').length)} color="green" />
      </div>
