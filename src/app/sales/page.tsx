...
function printInvoice(r: SaleRecord) {
  ...
  if (!w) return
  w.document.write(`<html><head><title>Invoice #${r.id.slice(0,6).toUpperCase()}</title>
  ...
  <div class="row"><span>Egg size</span><span>${r.size}</span></div>
  ...
  <br/><p style="color:#999;font-size:12px">Generated by Okesreal Farm Management System</p>
  </body></html>`)
        ...
        <>
          ...
          <div className="card">
            ...
            {sortedOther.length === 0 ? <EmptyState message="No other sales recorded yet." /> : (
              <div className="tbl-wrap">
                <table className="tbl">
                  <thead><tr><th>Date</th><th>Item</th><th>Qty</th><th>Unit</th><th>Unit price</th><th>Total</th><th>Customer</th><th>Status</th><th></th></tr></thead>
                  <tbody>
                    {sortedOther.map(r => (
                      <tr key={r.id}>
                        <td>{r.date}</td>
